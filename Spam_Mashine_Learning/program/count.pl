...
for($z=1230735600;$z<1262271600;$z+=$kankaku){
    ...
    for($i=0;$i<=$#splittime;$i++){
      ...
      print ECHO "$splittime[$i],$htmlnum[$i],$textnum[$i],$nonmultipart[$i],$multipart_text[$i],$multipart_nontext[$i],$spliturl[$i],$spliturlper[$i],$splitspam_word[$i],$splitspam_f[$i],$splitnonspam[$i],$splitspamspam[$i]\n";
